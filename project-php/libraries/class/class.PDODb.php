...
class PDODb
{
    ...
    /**
     * Store actual insert id
     * @var int
     */
    private $actualInsertId = 0;
    /**
    ...
    private function buildInsert($tableName, $insertData, $operation)
    {
        ...
        $this->actualInsertId = $this->pdo()->lastInsertId();

        $this->createLogs(@$_SESSION[$loginAdmin]['username'], 'insert', $tableName, $this->actualInsertId);
        $this->reset();
        if (
            $status && $this->actualInsertId > 0
        ) {
            return (int)$this->actualInsertId;
        }
        return $status;
    }
    /**
    ...
    public function createLogs($user = 'Unnamed', $act = '', $com = '', $id = 0)
    {
        ...
        if($com != 'history') {
            global $loginAdmin;
            $user_id = (isset($_SESSION[$loginAdmin]['id'])) ? $_SESSION[$loginAdmin]['id'] : 0;
            $title = is_string($id) ? $id : "ID: " . $id;
            
            try {
                $sql = "INSERT INTO " . $this->prefix . "history (user_id, username, action, module, title, created_at) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $this->pdo->prepare($sql);
                $stmt->execute([$user_id, $user, $act, $com, $title, time()]);
            } catch(Exception $e) {}
        }
    }
