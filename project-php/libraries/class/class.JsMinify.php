...
class JsMinify
{
    ...
    public function get()
    {
        ...
        foreach ($this->path as $index => $path) {
            $signature[] = $this->file[$index] . ':' . (file_exists($path['server']) ? filemtime($path['server']) : 0);
        }
        $this->init(md5(implode(",", $signature)));
    ...
    private function minify()
    {
        ...
        return '<script type="text/javascript" src="' . $this->access['asset'] . 'caches/serve.php?t=js&amp;f=' . $this->cacheName . '"></script>';
    }
    ...
    private function isExpire($file)
    {
        ...
        foreach ($this->path as $path) {
            if (file_exists($path['server']) && filemtime($path['server']) > $fileTime) {
                return true;
            }
        }
