...
class Comments
{
    ...
    private function syncProductRating($id_variant = 0, $type = '')
    {
        if (empty($id_variant) || $type !== 'san-pham') return;

        $row = $this->d->rawQueryOne(
            "select count(*) as total, avg(star) as average from #_comment where id_variant = ? and type = ? and id_parent = 0 and find_in_set('hienthi',status) and star between 1 and 5",
            array($id_variant, $type)
        );
        $total = !empty($row['total']) ? (int)$row['total'] : 0;
        $average = $total > 0 ? round((float)$row['average'], 1) : 0;

        if ($total === 0) {
            $product = $this->d->rawQueryOne("select rating from #_product where id = ? and type = ? limit 0,1", array($id_variant, $type));
            $average = !empty($product['rating']) ? round((float)$product['rating'], 1) : 5;
        }

        $this->d->where('id', $id_variant);
        $this->d->where('type', $type);
        $this->d->update('product', array(
            'rating_average' => max(1, min(5, $average)),
            'rating_count' => $total
        ));
    }
    ...
    {
        $row = $this->d->rawQueryOne("select count(id) as num from #_comment where find_in_set('hienthi',status) and id_parent = 0 and id_variant = ? and type = ? and star = ?", array($this->id_variant, $this->type, $star));
        return (!empty($row)) ? $row['num'] : 0;
    }

    private function totalStar()
    {
        $row = $this->d->rawQueryOne("select sum(star) as total_star from #_comment where find_in_set('hienthi',status) and id_parent = 0 and id_variant = ? and type = ? and star between 1 and 5", array($this->id_variant, $this->type));
        ...
        if (!empty($data)) {
            ...
            if (empty($this->errors)) {
                if ($this->d->insert('comment', $data)) {
                    ...
                    $this->syncProductRating($data['id_variant'] ?? 0, $data['type'] ?? '');
    ...
    public function status()
    {
        ...
        /* Get detail */
        if (!empty($id)) {
            $row = $this->d->rawQueryOne("select id, id_variant, type, id_parent, status from #_comment where id = ? limit 0,1", array($id));
            ...
            if (!empty($row['id'])) {
                ...
                } else if (empty($row['id_parent'])) {
                    $this->syncProductRating($row['id_variant'], $row['type']);
                }
            } else {
                $this->errors[] = dulieukhonghople;
            }
    ...
    public function delete()
    {
        ...
        /* Get detail */
        if (!empty($id)) {
            $row = $this->d->rawQueryOne("select id, id_parent, id_variant, type from #_comment where id = ? limit 0,1", array($id));
            ...
            if (!empty($row['id'])) {
                ...
                if (empty($row['id_parent'])) $this->syncProductRating($row['id_variant'], $row['type']);
            } else {
                $this->errors[] = dulieukhonghople;
            }
        } else {
