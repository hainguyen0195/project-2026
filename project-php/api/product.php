...
/* Get data */
$sql = "select name$lang, slugvi, slugen, id, photo,icon, regular_price, sale_price, discount, rating, rating_average, rating_count, type from #_product where type='san-pham' $where and find_in_set('noibat',status) and find_in_set('hienthi',status) order by numb,id desc";
...
<?php if ($countItems) { ?>
    <div class="row-product row row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-lg-3">
        <?= $func->getProductItems($items, $sluglang) ?>
    </div>
