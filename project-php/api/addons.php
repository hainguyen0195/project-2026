...
if ($type == 'footer-map') {
    $mapIframe = $func->decodeHtmlChars($optsetting['coords_iframe']);
    if (stripos($mapIframe, ' title=') === false) {
        $mapIframe = preg_replace('/<iframe\b/i', '<iframe title="Bản đồ vị trí công ty"', $mapIframe, 1);
    }
    echo $mapIframe;
}

if ($type == 'fanpage-facebook') { ?>
    <div class="fb-page" data-href="<?= $optsetting['fanpage'] ?>" data-tabs="timeline" data-width="501" data-height="215" data-small-header="false" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true">
