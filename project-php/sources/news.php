...
if ($id != '') {
    /* Lấy bài viết detail */
    $rowDetail = $d->rawQueryOne("select id, view, date_created, date_updated, id_list, id_cat, id_item, id_sub, type, name$lang, slugvi, slugen, desc$lang, content$lang, photo, options, info from #_news where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($id, $type));

    $articleInfo = !empty($rowDetail['info']) ? json_decode($rowDetail['info'], true) : array();
    if (!is_array($articleInfo)) $articleInfo = array();
    $aiArticle = (!empty($articleInfo['ai']) && is_array($articleInfo['ai'])) ? $articleInfo['ai'] : array();
    ...
    /* Lấy cấp 1 */
    $newsList = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_news_list where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_list'], $type));

    /* Lấy cấp 2 */
    $newsCat = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_news_cat where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_cat'], $type));

    /* Lấy cấp 3 */
    $newsItem = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_news_item where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_item'], $type));

    /* Lấy cấp 4 */
    $newsSub = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_news_sub where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_sub'], $type));

    /* Lấy bài viết cùng loại */
    $where = "";
    $where = "id <> ?  and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    ...
    /* Lấy tất cả bài viết */
    $where = "";
    $where = "type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    $params = array($type);
...
/* Blog listing: featured posts and level-1 category tabs. */
if ($id == '' && $type == 'blog') {
    $newsListTabs = $d->rawQuery(
        "select id, name$lang, slugvi, slugen from #_news_list where type = ? and find_in_set('hienthi',status) and (find_in_set('thungrac',status) = 0 or status is null) order by numb,id desc",
        array($type)
    );
    $featuredNews = $d->rawQuery(
        "select id, name$lang, slugvi, slugen, photo, desc$lang from #_news where type = ? and find_in_set('noibat',status) and find_in_set('hienthi',status) and (find_in_set('thungrac',status) = 0 or status is null) order by numb,id desc limit 0,5",
        array($type)
    );
    if (count($featuredNews) < 5) {
        $featuredIds = array_map(function ($item) { return (int)$item['id']; }, $featuredNews);
        $moreFeatured = $d->rawQuery(
            "select id, name$lang, slugvi, slugen, photo, desc$lang from #_news where type = ? and find_in_set('hienthi',status) and (find_in_set('thungrac',status) = 0 or status is null) order by numb,id desc limit 0,5",
            array($type)
        );
        foreach ($moreFeatured as $featuredItem) {
            if (!in_array((int)$featuredItem['id'], $featuredIds, true)) {
                $featuredNews[] = $featuredItem;
                $featuredIds[] = (int)$featuredItem['id'];
                if (count($featuredNews) >= 5) break;
            }
        }
    }
}
