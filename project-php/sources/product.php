...
$select = "id, name$lang, slugvi, slugen, photo, date_created, desc$lang, discount, sale_price, regular_price, rating, rating_average, rating_count";
$productSort = (!empty($_GET['sort'])) ? htmlspecialchars($_GET['sort']) : 'default';
$productOrder = "numb,id desc";
if ($productSort == 'newest') $productOrder = "date_created desc,id desc";
if ($productSort == 'price-asc') $productOrder = "regular_price asc,id desc";
if ($productSort == 'price-desc') $productOrder = "regular_price desc,id desc";
if ($productSort == 'name-asc') $productOrder = "name$lang asc,id desc";
if ($id != '') {
    /* Lấy sản phẩm detail */
    $rowDetail = $d->rawQueryOne("select type, id, name$lang, slugvi, slugen, desc$lang, content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, options, info, discount, sale_price, regular_price, rating, rating_average, rating_count, loiich$lang, thongsokythuat$lang, baogia$lang from #_product where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($id, $type));

    $productInfo = !empty($rowDetail['info']) ? json_decode($rowDetail['info'], true) : array();
    if (!is_array($productInfo)) $productInfo = array();
    $aiProduct = (!empty($productInfo['ai']) && is_array($productInfo['ai'])) ? $productInfo['ai'] : array();

    $productRatingManual = !empty($rowDetail['rating']) ? max(1, min(5, (float)$rowDetail['rating'])) : 5;
    $productRatingTotal = !empty($rowDetail['rating_count']) ? (int)$rowDetail['rating_count'] : 0;
    $productCommentType = !empty($rowDetail['type']) ? $rowDetail['type'] : $type;
    $productCommentEnabled = !empty($config['product'][$productCommentType]['comment']);
    $productRatingHasReviews = $productCommentEnabled && $productRatingTotal > 0;
    $productRatingAverage = $productRatingHasReviews && !empty($rowDetail['rating_average'])
        ? max(1, min(5, round((float)$rowDetail['rating_average'], 1)))
        : $productRatingManual;
    $productSoldRow = $d->rawQueryOne("select coalesce(sum(quantity),0) as total from #_order_detail where id_product = ?", array($id));
    $productSold = !empty($productSoldRow['total']) ? (int)$productSoldRow['total'] : 0;
    $productFlashSale = array();
    if ($d->tableExists('product_flash_sale')) {
        $productFlashSale = $d->rawQueryOne("select end_at,original_price,flash_price from #_product_flash_sale where product_id = ? and start_at <= NOW() and end_at > NOW() limit 0,1", array($id));
        if (!empty($productFlashSale) && $productFlashSale['original_price'] > 0 && $productFlashSale['flash_price'] > 0) {
            $rowDetail['regular_price'] = $productFlashSale['original_price'];
            $rowDetail['sale_price'] = $productFlashSale['flash_price'];
            $rowDetail['discount'] = round(100 - ($productFlashSale['flash_price'] * 100 / $productFlashSale['original_price']));
        }
    }

    $productSidebarNews = $d->rawQuery(
        "select id, name$lang, slugvi, slugen, photo, date_created from #_news where type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc limit 0,5",
        array('tin-tuc')
    );
    /* Product seen */
    ...
    if (!empty($productTags)) {
        $rowTags = $d->rawQuery("select id, name$lang, slugvi, slugen from #_tags where type='" . $type . "' and id in ($productTags) and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc");
    }
    /* Lấy màu */
    ...
    if (!empty($productColor)) {
        $rowColor = $d->rawQuery("select type_show, photo, color, id from #_color where type='" . $type . "' and id in ($productColor) and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc");
    }
    /* Lấy size */
    ...
    if (!empty($productSize)) {
        $rowSize = $d->rawQuery("select id, name$lang from #_size where type='" . $type . "' and id in ($productSize) and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc");
    }
    /* Lấy cấp 1 */
    $productList = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_product_list where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_list'], $type));
    /* Lấy cấp 2 */
    $productCat = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_product_cat where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_cat'], $type));
    /* Lấy cấp 3 */
    $productItem = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_product_item where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_item'], $type));
    /* Lấy cấp 4 */
    $productSub = $d->rawQueryOne("select id, name$lang, slugvi, slugen from #_product_sub where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) limit 0,1", array($rowDetail['id_sub'], $type));
    /* Lấy thương hiệu */
    $productBrand = $d->rawQueryOne("select name$lang, slugvi, slugen, id from #_product_brand where id = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)", array($rowDetail['id_brand'], $type));
    /* Lấy hình ảnh con */
    $rowDetailPhoto = $d->rawQuery("select photo from #_gallery where id_parent = ? and com='product' and type = ? and kind='man' and val = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc", array($rowDetail['id'], $type, $type));
    /* Lấy sản phẩm cùng loại */
    $where = "";
    $where = "id <> ? and id_list = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    $params = array($id, $rowDetail['id_list'], $type);
    $curPage = $getPage;
    $perPage = 8;
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select $select from #_product where $where order by $productOrder $limit";
    ...
    /* Lấy cấp 1 detail */
    $productList = $d->rawQueryOne("select id, name$lang, slugvi, slugen, type, photo, options, desc$lang, content$lang from #_product_list where id = ? and type = ? limit 0,1", array($idl, $type));
    /* SEO */
    ...
    /* Lấy sản phẩm */
    $where = "";
    $where = "id_list = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    ...
    /* Lấy sản phẩm */
    $where = "";
    $where = "id_cat = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    ...
    /* Lấy sản phẩm */
    $where = "";
    $where = "id_item = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    ...
    /* Lấy sản phẩm */
    $where = "";
    $where = "id_sub = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    ...
    /* Lấy sản phẩm */
    $where = "";
    $where = "id_brand = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)";
    ...
    /* Trang tổng quan: sidebar và sản phẩm được chia theo danh mục cấp 1. */
    $productOverview = true;
    $productOverviewGroups = $d->rawQuery(
        "select id, name$lang, desc$lang, slugvi, slugen from #_product_list where type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc",
        array($type)
    );

    foreach ($productOverviewGroups as $key => $group) {
        $groupCount = $d->rawQueryOne(
            "select count(*) as num from #_product where id_list = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL)",
            array($group['id'], $type)
        );
        $productOverviewGroups[$key]['total'] = !empty($groupCount['num']) ? (int)$groupCount['num'] : 0;
        $productOverviewGroups[$key]['products'] = $d->rawQuery(
            "select $select from #_product where id_list = ? and type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by $productOrder limit 0,8",
            array($group['id'], $type)
        );
    }
...
/* Sidebar for product category levels */
if (!empty($idl) || !empty($idc) || !empty($idi) || !empty($ids)) {
    $productSidebarNews = $d->rawQuery(
        "select id, name$lang, slugvi, slugen, photo, date_created from #_news where type = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc limit 0,5",
        array('tin-tuc')
    );
