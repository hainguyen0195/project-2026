...
$select = "id, name$lang, slugvi, slugen, photo, date_created, desc$lang, address$lang";
...
if ($id != '') {
    ...
    $rowDetailPhoto = $d->rawQuery("select photo from #_gallery where id_parent = ? and com='news' and type = ? and kind='man' and val = ? and find_in_set('hienthi',status) and (FIND_IN_SET('thungrac',status) = 0 OR status IS NULL) order by numb,id desc", array($rowDetail['id'], $type, $type));
    ...
    $sql = "select $select from #_news where $where order by numb,id desc $limit";
