...
@session_start();
if ($_SESSION['dongdau']!='') {
	$wmi = '/upload/hinhanh/'.$_SESSION['dongdau'];
}
else
{
	$d->reset();
    ...
    $wmi = '/upload/hinhanh/'.$dongdau['photo'];
}
/**
...
if($_GET['w'] < 600){
	if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', dirname(FILE) . $wmi ); // the watermark image
} else {
	if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', dirname(FILE) . $wmi ); // the watermark image
}
...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './cachewater');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
